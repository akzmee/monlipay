...
describe("ConnectButton", () => {
  ...
  describe("when disconnected", () => {
    ...
    it("should NOT call openConnectModal when the online indicator is clicked (it opens the network modal instead)", () => {
      const { container } = render(<ConnectButton />);
      const indicator = container.querySelector(".bg-emerald-500");
    ...
    it("should open the network switcher modal when online indicator is clicked", () => {
      const { container } = render(<ConnectButton />);
      const indicator = container.querySelector(".bg-emerald-500");
      fireEvent.click(indicator!.closest("button")!);
      // Modal should be visible with both networks listed.
      expect(
  ...
  describe("when connected on correct chain (Monad Testnet)", () => {
    ...
    it("should show green dot indicator", () => {
      const { container } = render(<ConnectButton />);
      const indicator = container.querySelector(".bg-emerald-500");
      expect(indicator).toBeInTheDocument();
    });
    ...
    it("should open network switcher modal when chain dot indicator is clicked (not openChainModal)", () => {
      const { container } = render(<ConnectButton />);
      const indicator = container.querySelector(".bg-emerald-500");
      fireEvent.click(indicator!.closest("button")!);
